...
def get_holding_position(code):
    ...
    if ret != RET_OK:
        ...
        logger.info('【持仓状态】 {} 的持仓数量为：{}'.format(TRADING_SECURITY, holding_position))
...
def on_bar_open():
    ...
    if holding_position == 0:
        if bull_or_bear == 1:
            logger.info('【操作信号】 做多信号，建立多单。')
            open_position(TRADING_SECURITY)
        else:
            logger.info('【操作信号】 做空信号，不开空单。')
    elif holding_position > 0:
        if bull_or_bear == -1:
            logger.info('【操作信号】 做空信号，平掉持仓。')
            close_position(TRADING_SECURITY, holding_position)
        else:
            logger.info('【操作信号】 做多信号，无需加仓。')
