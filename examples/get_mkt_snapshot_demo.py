...
def loop_get_mkt_snapshot(api_svr_ip, api_svr_port, market):
    ...
    for sub_type in stock_type:
        ...
        if ret_code == 0:
            logger.info("get_stock_basicinfo: market={}, sub_type={}, count={}".format(market, sub_type, len(ret_data)))
            for ix, row in ret_data.iterrows():
                stock_codes.append(row['code'])

    if len(stock_codes) == 0:
        quote_ctx.close()
        logger.info("Error market:'{}' can not get stock info".format(market))
        return

    # 按频率限制获取股票快照: 每3秒200支股票
    for i in range(1, len(stock_codes), 200):
        logger.info(f"from {i}, total {len(stock_codes)}")
...
if __name__ == "__main__":
    ...
    for mkt in [ft.Market.HK, ft.Market.US]:
