...
def simple_sell(quote_ctx, trade_ctx, stock_code, trade_price, volume, trade_env, order_type=ft.OrderType.NORMAL):
    ...
    while True:
        ...
        if lot_size == 0:
            ...
            if ret != ft.RET_OK:
                logger.info("can't get lot size, retrying: {}".format(data))
                continue
            elif lot_size <= 0:
        ...
        if ret != ft.RET_OK:
            logger.info('simple_sell 下单失败:{}'.format(data))
            return None
        else:
            logger.info('simple_sell 下单成功')
            return data
...
def smart_sell(quote_ctx, trade_ctx, stock_code, volume, trade_env, order_type=ft.OrderType.NORMAL):
    ...
    while True:
        ...
        if ret != ft.RET_OK:
            logger.info("can't get orderbook, retrying:{}".format(data))
            continue

        price = data['Bid'][0][0]  # 取得买一价
        logger.info('smart_sell bid price is {}'.format(price))

        # 以买一价下单卖出
        ret, data = trade_ctx.place_order(price=price, qty=qty, code=stock_code,
                                          trd_side=ft.TrdSide.SELL, trd_env=trade_env, order_type=order_type)
        if ret != ft.RET_OK:
            logger.info('smart_sell 下单失败:{}'.format(data))
            return None
        else:
            logger.info('smart_sell 下单成功')
            logger.info(data)
            return data
...
if __name__ =="__main__":
    ...
    if ret == ft.RET_OK:
        ...
        logger.info("* unlock_trade fail: ", data)
