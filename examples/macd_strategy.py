...
class MACD(object):
    ...
    def handle_data(self):
        ...
        if ret_code != ft.RET_OK:
            logger.info("request_history_kline fail: {}".format(prices))
            return
        ...
        if macd[-1] < signal[-1] and macd[-2] > signal[-2]:
            ...
            if cur_pos > 0:
                ...
                ret_code, ret_data = self.trade_ctx.place_order(
                    ...
                    trd_env=self.trade_env)
                if ret_code == ft.RET_OK:
                    logger.info('stop_loss MAKE SELL ORDER\n\tcode = {} price = {} quantity = {}'
                          .format(self.stock, cur_price, cur_pos))
                else:
                    logger.info('stop_loss: MAKE SELL ORDER FAILURE: {}'.format(ret_data))
        ...
        if macd[-1] > signal[-1] and macd[-2] < signal[-2]:
            ...
            if not ret_code:
                ...
                logger.info('stop_loss: MAKE BUY ORDER FAILURE: {}'.format(ret_data))
